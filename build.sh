...
build_nix() {
  ...
  rm -rf "$nix_dist"
  mkdir -p "$nix_dist"
  cp packaging/nix/flake.nix packaging/nix/default.nix "$nix_dist/"
...
EOF
  if command -v nix >/dev/null 2>&1; then
    ...
    if (cd "$ROOT_DIR" && nix build ./packaging/nix#default --out-link "$DIST_DIR/nix-result" 2>"$DIST_DIR/nix-build.log"); then
      ok "dérivation Nix construite: dist/nix-result"
    else
      warn "build Nix échouée ou nix non configuré pour les flakes (voir dist/nix-build.log). Les fichiers .nix sont fournis pour build sur une machine Nix/NixOS."
    fi
  else
    warn "nix n'est pas installé sur cette machine : fichiers flake.nix/default.nix fournis tels quels dans dist/${APP_NAME}-${VERSION}-nix/ pour être construits sur NixOS/GLF OS."
  fi
