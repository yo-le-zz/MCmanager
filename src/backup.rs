...
pub fn create_backup(data_dir: &Path, server_id: &uuid::Uuid, server_folder: &Path) -> Result<String> {
    ...
    for entry in WalkDir::new(server_folder).into_iter().filter_map(|e| e.ok()) {
        ...
        // (the zip spec requires it, and on Windows `to_string_lossy()`
        // would otherwise emit backslashes, breaking extraction on other
        // platforms and the logs/crash-reports filter below).
